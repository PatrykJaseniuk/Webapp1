-- ================================================
-- RENTAL MANAGEMENT SYSTEM - SEED DATA (PART 3)
-- ================================================
-- Part 3: More Transactions and Attachments
-- This file is split to keep migration files under 400 lines

-- ================================================
-- SECTION 7: TRANSACTIONS (CONTINUED)
-- ================================================

-- ===== LEASE 2: Kraków Apartment (Anna Nowak) =====
INSERT INTO public.transactions (id, lease_id, property_id, description, amount, due_date, created_at, updated_at, created_by) VALUES
    -- Deposit
    ('d0000000-0000-0000-0000-000000000100', 
     'c0000000-0000-0000-0000-000000000002', 
     'a0000000-0000-0000-0000-000000000002', 
     'Security deposit', 
     4200.00, 
     '2025-03-01', 
     '2025-02-25 14:00:00+00', 
     '2025-03-01 14:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    -- Rent payments (negative amounts - expense from landlord perspective)
    ('d0000000-0000-0000-0000-000000000101', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - March 2025', 
     -4200.00, 
     '2025-03-01', 
     '2025-03-01 09:00:00+00', 
     '2025-03-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000102', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - April 2025', 
     -4200.00, 
     '2025-04-01', 
     '2025-04-01 09:00:00+00', 
     '2025-04-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000103', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - May 2025', 
     -4200.00, 
     '2025-05-01', 
     '2025-05-01 09:00:00+00', 
     '2025-05-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000104', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - June 2025', 
     -4200.00, 
     '2025-06-01', 
     '2025-06-01 09:00:00+00', 
     '2025-06-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000105', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - July 2025', 
     -4200.00, 
     '2025-07-01', 
     '2025-07-01 09:00:00+00', 
     '2025-07-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000106', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - August 2025', 
     -4200.00, 
     '2025-08-01', 
     '2025-08-01 09:00:00+00', 
     '2025-08-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000107', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - September 2025', 
     -4200.00, 
     '2025-09-01', 
     '2025-09-01 09:00:00+00', 
     '2025-09-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000108', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - October 2025', 
     -4200.00, 
     '2025-10-01', 
     '2025-10-01 09:00:00+00', 
     '2025-10-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000109', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - November 2025', 
     -4200.00, 
     '2025-11-01', 
     '2025-11-01 09:00:00+00', 
     '2025-11-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000110', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - December 2025', 
     -4200.00, 
     '2025-12-01', 
     '2025-12-01 09:00:00+00', 
     '2025-12-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000111', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - January 2026', 
     -4200.00, 
     '2026-01-01', 
     '2026-01-01 09:00:00+00', 
     '2026-01-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000112', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Monthly rent - February 2026', 
     -4200.00, 
     '2026-02-01', 
     '2026-02-01 09:00:00+00', 
     '2026-02-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    -- Utility bills for Kraków (negative amounts - expense)
    ('d0000000-0000-0000-0000-000000000113', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Electricity - March 2025', 
     -85.20, 
     '2025-04-10', 
     '2025-04-10 10:00:00+00', 
     '2025-04-10 10:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000114', 
     'c0000000-0000-0000-0000-000000000002', 
     NULL, 
     'Gas - March 2025', 
     -120.00, 
     '2025-04-10', 
     '2025-04-10 10:00:00+00', 
     '2025-04-10 10:00:00+00', 
     '00000000-0000-0000-0000-000000000002');

-- ===== LEASE 3: Gdańsk House (Piotr Wiśniewski) =====
INSERT INTO public.transactions (id, lease_id, property_id, description, amount, due_date, created_at, updated_at, created_by) VALUES
    -- Deposit
    ('d0000000-0000-0000-0000-000000000200', 
     'c0000000-0000-0000-0000-000000000003', 
     'a0000000-0000-0000-0000-000000000003', 
     'Security deposit', 
     6500.00, 
     '2024-01-01', 
     '2023-12-15 09:00:00+00', 
     '2024-01-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    -- Rent payments (negative amounts - expense from landlord perspective)
    ('d0000000-0000-0000-0000-000000000201', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - January 2025', 
     -6500.00, 
     '2025-01-01', 
     '2025-01-01 09:00:00+00', 
     '2025-01-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000202', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - February 2025', 
     -6500.00, 
     '2025-02-01', 
     '2025-02-01 09:00:00+00', 
     '2025-02-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000203', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - March 2025', 
     -6500.00, 
     '2025-03-01', 
     '2025-03-01 09:00:00+00', 
     '2025-03-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000204', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - April 2025', 
     -6500.00, 
     '2025-04-01', 
     '2025-04-01 09:00:00+00', 
     '2025-04-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000205', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - May 2025', 
     -6500.00, 
     '2025-05-01', 
     '2025-05-01 09:00:00+00', 
     '2025-05-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000206', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - June 2025', 
     -6500.00, 
     '2025-06-01', 
     '2025-06-01 09:00:00+00', 
     '2025-06-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000207', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - July 2025', 
     -6500.00, 
     '2025-07-01', 
     '2025-07-01 09:00:00+00', 
     '2025-07-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000208', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - August 2025', 
     -6500.00, 
     '2025-08-01', 
     '2025-08-01 09:00:00+00', 
     '2025-08-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000209', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - September 2025', 
     -6500.00, 
     '2025-09-01', 
     '2025-09-01 09:00:00+00', 
     '2025-09-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000210', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - October 2025', 
     -6500.00, 
     '2025-10-01', 
     '2025-10-01 09:00:00+00', 
     '2025-10-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000211', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - November 2025', 
     -6500.00, 
     '2025-11-01', 
     '2025-11-01 09:00:00+00', 
     '2025-11-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000212', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - December 2025', 
     -6500.00, 
     '2025-12-01', 
     '2025-12-01 09:00:00+00', 
     '2025-12-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000213', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - January 2026', 
     -6500.00, 
     '2026-01-01', 
     '2026-01-01 09:00:00+00', 
     '2026-01-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002'),
    
    ('d0000000-0000-0000-0000-000000000214', 
     'c0000000-0000-0000-0000-000000000003', 
     NULL, 
     'Monthly rent - February 2026', 
     -6500.00, 
     '2026-02-01', 
     '2026-02-01 09:00:00+00', 
     '2026-02-01 09:00:00+00', 
     '00000000-0000-0000-0000-000000000002');
