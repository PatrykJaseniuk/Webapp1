...
export type LeaseAgreementsSProps = ManyRecordsSlaveProps<LeaseAgreementRow, LeaseAgreementSortColumn, NavLinkTo> &
  Readonly<{
    readonly filterValues: LeaseAgreementsFilterValues;
    readonly onFilterChange: (patch: Partial<LeaseAgreementsFilterValues>) => void;
...
export const LeaseAgreementsM = ({
  ...
  const [filterValues, setFilterValues] = useState<LeaseAgreementsFilterValues>(INITIAL_FILTER_VALUES);
  const [filterPanelOpen, setFilterPanelOpen] = useState(false);
  const toggleFilterPanel = useCallback((): void => setFilterPanelOpen((p) => !p), []);

  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const searchRef = useRef<string | null>(null);

  const applyFilterPatch = useCallback(
    (patch: Partial<LeaseAgreementsFilterValues>): void => {
      const isSearchOnly = 'search' in patch && Object.keys(patch).length === 1;
      debounceRef.current !== null && clearTimeout(debounceRef.current);
      isSearchOnly ?
        // eslint-disable-next-line functional/immutable-data -- standard React useRef mutable pattern
        (debounceRef.current = setTimeout(() => {
          // eslint-disable-next-line functional/immutable-data -- standard React useRef mutable pattern
          searchRef.current = patch.search ?? null;
          setFilterValues((prev) => ({ ...prev, search: searchRef.current }));
        }, DEBOUNCE_MS)) :
  ...
  const onClearFilters = useCallback((): void => setFilterValues(INITIAL_FILTER_VALUES), []);

  const extraQueryKey = [filterValues.leaseStatus, filterValues.startDateFrom, filterValues.startDateTo, filterValues.search];
