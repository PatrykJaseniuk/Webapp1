...
export const ViewSingleProperty = ({ id }: ViewSinglePropertyProps) => {
    ...
    const [createState, handleCreate] = useAsyncFn(async () => {
        ...
        return await database.from('properties').insert(scalarFields as any).select().single();
    }, [formState]);
    ...
    const [updateState, handleUpdate] = useAsyncFn(async () => {
        ...
        return await database.from('properties').update(scalarFields as any).eq('id', id!).select().single();
    }, [formState, id]);

    // Delete mutation
    const [deleteState, handleDelete] = useAsyncFn(async () => {
        return await database.from('properties').delete().eq('id', id!);
    }, [id]);
    ...
    const handleSave = () => {
        if (isCreateMode) {
            handleCreate().then((result) => {
                result?.data && router.push(routes.landlord.properties({ id: result.data.id as string }));
            });
        } else {
    ...
    const handleCancel = () => {
        if (isCreateMode) {
            router.push(routes.landlord.properties());
        } else {
            setMode('view');
    ...
    const handleDeleteConfirm = () => {
        handleDelete().then((result) => {
            !result?.error && router.push(routes.landlord.properties());
        });
    };
