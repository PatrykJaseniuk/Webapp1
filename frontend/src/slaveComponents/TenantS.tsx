      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div>
            ...
            <span className={statusPillClass(t.tenant_status)}>{STATUS_LABEL[t.tenant_status]}</span>
          </div>
          <div>
            <p className={labelClass}>Email</p>
            <p className={valueClass}>{t.email}</p>
          </div>
          <div>
            <p className={labelClass}>Telefon</p>
            <p className={valueClass}>{t.phone}</p>
          </div>
          <div>
            <p className={labelClass}>Nr dokumentu</p>
            <p className={valueClass}>{t.id_document_number ?? '—'}</p>
          </div>
          <div>
            <p className={labelClass}>Kontakt awaryjny (imię i nazwisko)</p>
            <p className={valueClass}>{t.emergency_contact_name ?? '—'}</p>
          </div>
          <div>
            <p className={labelClass}>Kontakt awaryjny (telefon)</p>
            <p className={valueClass}>{t.emergency_contact_phone ?? '—'}</p>
          </div>
        </div>
        {t.notes !== null ?
          <div className="mt-4">
            <p className={labelClass}>Notatki</p>
            <p className={`${valueClass} mt-1 whitespace-pre-wrap`}>{t.notes}</p>
          </div> :
          undefined}
      </div>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 text-gray-500">
                  <th className="py-2 pr-4 font-medium">Nieruchomość</th>
