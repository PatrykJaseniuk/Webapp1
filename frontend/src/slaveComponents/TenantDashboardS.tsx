...
const StatCards = ({ summary }: { readonly summary: DashboardSummary }): JSX.Element => (
  <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
    <div className={statCardClass}>
      <p className={`${statValueClass} text-blue-700`}>{summary.totalProperties}<span className="ml-1 text-sm font-normal text-gray-400">/ {summary.occupiedProperties} zajętych</span></p>
      <p className={statLabelClass}>Nieruchomości</p>
    </div>
    <div className={statCardClass}>
      <p className={`${statValueClass} text-green-700`}>{summary.totalTenants}<span className="ml-1 text-sm font-normal text-gray-400">/ {summary.activeTenants} aktywnych</span></p>
      <p className={statLabelClass}>Najemcy</p>
    </div>
    <div className={statCardClass}>
      <p className={`${statValueClass} text-red-700`}>{summary.overdueItems}</p>
      <p className={statLabelClass}>Zaległe pozycje</p>
    </div>
    <div className={statCardClass}>
      <p className={`${statValueClass} text-amber-700`}>{summary.totalUnpaidAmount.toLocaleString('pl-PL')} zł</p>
      <p className={statLabelClass}>Nieopłacone</p>
    </div>
  </div>
);

export const TenantDashboard = ({ cards, asyncData }: Props): JSX.Element => (
